...
if __name__ == "__main__":
    ...
    usecase = GetContentTgUseCase(gateway, nlp_processor, None, None)
    contents = usecase.execute()
    if contents:
        print("Результаты работы usecase:")
        for content in contents:
            # Для pydantic v2 используйте model_dump_json()
            print(content.model_dump_json())
    else:
        print("Новых сообщений не найдено или обработка не дала результата.")
