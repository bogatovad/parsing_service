...
class KudaGoGateway(BaseGateway):
    ...
    def _parse_time(self, event_details: dict):
        ...
        if len(event_details["dates"]) == 1:
            ...
            end_time_none = (
                event_date["end_time"] is None or event_date["end_time"] == "00:00:00"
            )

            start_time_midnight = (
                True
                if (
                    event_date.get("start_time") == "00:00:00"
                    or event_date.get("start_time") is None
                )
                else False
            )
            ...
            none_condition = (
                True
                if (start_date_none and end_date_none)
                or (start_date_none and start_time_midnight and end_time_none)
                else False
            )

            if (
                use_schedule is False
                and none_condition is True
                and event_date["schedules"] == []
            ):
                ...
                none_condition = (
                    True
                    if (start_date_none and end_date_none)
                    or (start_date_none and start_time_midnight)
                    else False
                )

                if not (start_date_none and end_date_none) or not (
                    start_date_none and start_time_midnight
                ):
            ...
            try:
                ...
                for date in event_details["dates"]:
                    if date["schedules"] != [] and date["is_endless"] is True:
                        ...
                        for n, schedule in enumerate(schedules):
                            ...
                            end_time = (
                                True if schedule["end_time"] is not None else False
                            )
                            if end_time:
                                schedule_t_string = f"{days_of_week}: {schedule['start_time'].split(':')[0]}:{schedule['start_time'].split(':')[1]}-{schedule['end_time'].split(':')[0]}:{schedule['end_time'].split(':')[1]}"
                            else:
                            ...
                            if schedule_list != []:
                                ...
                                already_present_in_list = (
                                    True
                                    if True
                                    in [
                                        True
                                        for this_string in schedule_list
                                        if what_presents in this_string
                                    ]
                                    else False
                                )
                                ...
                                already_present = (
                                    ...
                                    in [
                                        True
                                        for day in week_day.values()
                                        if day in schedule_t_string
                                        and already_present_in_list
                                    ]
                                    else False
                                )

                                if (
                                    schedule_t_string not in schedule_list
                                    ...
                                    schedule_list[index_position] = (
                                        f"{what_presents}: {current_substring}, {to_append}"
                                    )
                            else:
                                schedule_list.append(schedule_t_string.lower())
                ...
                if (
                    len(schedule_list) == 1
                    and not event_details["dates"][0]["is_endless"]
                ):
                    schedule_list[0] = (
                        schedule_list[0].split(" с ")[1]
