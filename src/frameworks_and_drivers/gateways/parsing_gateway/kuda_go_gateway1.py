...
class EventParser:
    ...
    def parse_event(self, event):
        ...
        if details["place"] == None:  # noqa: E711
            ...
            details["place"] = (
                {"title": " ", "address": matches[0], "phone": "-"}
                if len(matches) > 0
                else {
                    "title": " ",
                    "address": "Уточняйте у организаторов",
                    "phone": "-",
                }
            )
    ...
    def _parse_schedule(self, event_details: dict):
        ...
        for date in event_details.get("dates", []):
            ...
            if date.get("schedules"):
                for schedule in date["schedules"]:
                    days_str = get_days_range(
                        schedule.get("days_of_week", []), week_day
                    )
                    entry = create_schedule_entry(
                        days_str, schedule.get("start_time"), schedule.get("end_time")
                    )
                    if entry:
        ...
        if "Стоимость" in body_text:
            matches = re.findall(r"!Время:([^\n]+)\nСтоимость:([^\n]+)", body_text)
            if matches:
                backup_schedule = [
                    f"{t.strip()} (Стоимость: {p.strip()})" for t, p in matches
                ]
