...
class Text2ImageAPI:
    ...
    def generate(self, prompt: str, model_id: str, images: int = 1, width: int = 512, height: int = 512) -> str:
        params = {
            "type": "GENERATE",
            "numImages": images,
            "width": width,
        ...
        if response.status_code != 201:
            raise Exception(f"Ошибка при запуске генерации: {response.status_code}, {response.text}")
        return response.json()['uuid']

    def check_generation(self, request_id: str, attempts: int = 20, delay: int = 5) -> list:
...
def generate_image_with_kandinsky(prompt: str) -> bytes:
    ...
    # Сохраняем временно файл, затем удаляем (если нужно)
    file_name = f"kandinsky_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
    with open(file_name, 'wb') as f:
        f.write(image_data)
    logging.info(f"Временное изображение сохранено как {file_name}")
    if os.path.exists(file_name):
        os.remove(file_name)
        logging.info(f"Временный файл {file_name} удалён.")
    return image_data
