...
def is_duplicate(
    ...
    fuzz_threshold_location=85,
) -> bool:
    same_date = new_evt["date"] == existing_evt["date"]
    same_time = (
        new_evt["time"] == existing_evt["time"]
        and new_evt["time"] != ""
        and existing_evt["time"] != ""
    )

    location_similarity = fuzz.token_set_ratio(
        normalize_fuzzy(new_evt["location"]), normalize_fuzzy(existing_evt["location"])
    )
    same_loc = location_similarity >= fuzz_threshold_location

    name_similarity = fuzz.token_set_ratio(
        normalize_fuzzy(new_evt["name"]), normalize_fuzzy(existing_evt["name"])
    )
...
def check_and_add_event(
    new_event_raw: dict,
    log_path: str = DEDUP_LOG_PATH,
    fuzz_threshold_name: int = 80,
    fuzz_threshold_location: int = 85,
) -> bool:
